...
interface RouteContext {
  params: {
    jobId: string; // CHANGED: Use jobId to match attachments route
  };
}
...
export async function GET(request: NextRequest, { params }: RouteContext) {
  const { jobId } = params; // CHANGED: Use jobId instead of jobNumber

  console.log(`=== SIMPRO JOB DETAILS REQUEST ===`);
  console.log(`Job ID: ${jobId}`); // CHANGED: Updated log message
  ...
  try {
    ...
    const jobDetails = await apiRequest(jobUrl);
